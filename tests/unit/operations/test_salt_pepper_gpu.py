"""Tests for GPU-accelerated salt and pepper noise operation (Mac only)."""
...
class TestSaltPepperGPUOperation:
    """Tests for SaltPepperGPUOperation class."""

    @pytest.fixture
    def operation(self) -> SaltPepperGPUOperation:
        """Create a GPU salt and pepper operation instance."""
        return SaltPepperGPUOperation()
    ...
    def test_operation_name(self, operation: SaltPepperGPUOperation) -> None:
        """Test operation has correct name."""
        assert operation.name == "salt_pepper_gpu"

    def test_valid_params(self, operation: SaltPepperGPUOperation) -> None:
        """Test valid parameter validation."""
        params = {"amount": 0.05, "salt_vs_pepper": 0.5}
        operation.validate_params(params)  # Should not raise

    def test_valid_params_with_seed(self, operation: SaltPepperGPUOperation) -> None:
        """Test valid parameters with seed."""
        params = {"amount": 0.1, "salt_vs_pepper": 0.7, "seed": 42}
        operation.validate_params(params)  # Should not raise

    def test_missing_amount_raises_error(
        self, operation: SaltPepperGPUOperation
    ) -> None:
    ...
    def test_apply_basic(
        self, operation: SaltPepperGPUOperation, test_image: Image.Image
    ) -> None:
        """Test applying GPU salt and pepper noise."""
    ...
    def test_apply_produces_noise(
        ...
        """Test that GPU noise is actually added."""
        params = {"amount": 0.1, "salt_vs_pepper": 0.5, "seed": 42}
        result = operation.apply(test_image, params)
    ...
    def test_apply_grayscale(self, operation: SaltPepperGPUOperation) -> None:
    ...
    def test_gpu_matches_cpu_results(self, test_image: Image.Image) -> None:
        """Test that GPU and CPU versions produce identical results with same seed."""
    ...
    def test_gpu_performance_better_than_cpu(self) -> None:
        """Test that GPU version is faster than CPU for large images."""
