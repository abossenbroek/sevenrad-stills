"""Tests for GPU-accelerated downscale operation (Mac only)."""
...
class TestDownscaleGPUOperation:
    """Tests for DownscaleGPUOperation class."""

    @pytest.fixture
    def operation(self) -> DownscaleGPUOperation:
        """Create a GPU downscale operation instance."""
        return DownscaleGPUOperation()
    ...
    def test_operation_name(self, operation: DownscaleGPUOperation) -> None:
        """Test operation has correct name."""
        assert operation.name == "downscale_gpu"

    def test_valid_params(self, operation: DownscaleGPUOperation) -> None:
        """Test valid parameter validation."""
        params = {"scale": 0.5}
        operation.validate_params(params)  # Should not raise

    def test_valid_params_with_all_options(
        self, operation: DownscaleGPUOperation
    ) -> None:
    ...
    def test_missing_scale_raises_error(self, operation: DownscaleGPUOperation) -> None:
    ...
    def test_scale_too_low_raises_error(self, operation: DownscaleGPUOperation) -> None:
    ...
    def test_unsupported_downscale_method_raises_error(
        ...
        with pytest.raises(ValueError, match="GPU version supports"):
            operation.validate_params(params)
    ...
    def test_apply_basic(
        self, operation: DownscaleGPUOperation, test_image: Image.Image
    ) -> None:
        """Test applying GPU downscale."""
        params = {"scale": 0.5}
    ...
    def test_apply_produces_change(
        ...
        """Test that GPU downscale actually changes the image."""
    ...
    def test_rgba_preserves_alpha(self, operation: DownscaleGPUOperation) -> None:
    ...
    def test_grayscale_raises_error(self, operation: DownscaleGPUOperation) -> None:
        """Test that grayscale images raise an error."""
        gray_image = Image.new("L", (100, 100), color=128)
        params = {"scale": 0.5}

        with pytest.raises(ValueError, match="GPU downscale requires RGB or RGBA"):
            operation.apply(gray_image, params)

    def test_extreme_downscale(self, operation: DownscaleGPUOperation) -> None:
    ...
    def test_no_downscale(
        ...
        assert np.mean(diff) < 1.0  # Average difference less than 1 pixel value

    def test_multiple_channels(self, operation: DownscaleGPUOperation) -> None:
