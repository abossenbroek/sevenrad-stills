"""Tests for Metal-accelerated circular blur operation."""
...
@pytest.fixture
def blur_op_metal() -> CircularBlurMetalOperation:
    """Create a Metal circular blur operation instance."""
    return CircularBlurMetalOperation()
...
class TestCircularBlurMetalValidation:
    """Test parameter validation for Metal circular blur operation."""

    def test_validation_missing_radius(
        self, blur_op_metal: CircularBlurMetalOperation
    ) -> None:
    ...
    def test_validation_invalid_radius_type(
        ...
        with pytest.raises(ValueError, match="Radius must be an integer"):
            blur_op_metal.validate_params({"radius": 2.5})
    ...
    def test_validation_negative_radius(
        ...
        with pytest.raises(ValueError, match="Radius must be non-negative"):
            blur_op_metal.validate_params({"radius": -1})
    ...
    def test_validation_valid_radius(
        ...
        # Should not raise
        blur_op_metal.validate_params({"radius": 5})
        blur_op_metal.validate_params({"radius": 0})
        blur_op_metal.validate_params({"radius": 10})


class TestCircularBlurMetalApply:
    """Test applying Metal circular blur to images."""

    def test_apply_zero_radius(
        self, blur_op_metal: CircularBlurMetalOperation, test_image_rgb: Image.Image
    ) -> None:
        """Test that radius=0 returns an identical image."""
        result = blur_op_metal.apply(test_image_rgb, {"radius": 0})
    ...
    def test_apply_rgb_image(
        ...
        result = blur_op_metal.apply(test_image_rgb, {"radius": 3})
    ...
    def test_apply_grayscale_image(
        self,
        blur_op_metal: CircularBlurMetalOperation,
        test_image_grayscale: Image.Image,
    ) -> None:
        """Test applying blur to grayscale image."""
        result = blur_op_metal.apply(test_image_grayscale, {"radius": 3})
    ...
    def test_apply_rgba_image(
        self, blur_op_metal: CircularBlurMetalOperation, test_image_rgba: Image.Image
    ) -> None:
        """Test applying blur to RGBA image preserves alpha channel."""
        result = blur_op_metal.apply(test_image_rgba, {"radius": 3})
    ...
    def test_apply_various_radius_values(
        ...
        result_small = blur_op_metal.apply(test_image_rgb, {"radius": 2})
        result_large = blur_op_metal.apply(test_image_rgb, {"radius": 8})
    ...
    def test_operation_name(self, blur_op_metal: CircularBlurMetalOperation) -> None:
        """Test that operation has correct name."""
        assert blur_op_metal.name == "blur_circular_metal"
...
class TestMetalvsCPUConsistency:
    ...
    def test_rgb_consistency(
        ...
        radius = 5
        result_metal = blur_op_metal.apply(test_image_rgb, {"radius": radius})
    ...
    def test_grayscale_consistency(
        ...
        radius = 5
        result_metal = blur_op_metal.apply(test_image_grayscale, {"radius": radius})
    ...
    def test_rgba_consistency(
        ...
        radius = 5
        result_metal = blur_op_metal.apply(test_image_rgba, {"radius": radius})
    ...
    def test_small_radius_consistency(
        ...
        for radius in [1, 2, 3]:
            ...
            np.testing.assert_allclose(
                ...
                atol=1.0,
                rtol=0.01,
                err_msg=f"Metal and CPU should match for radius={radius}",
            )
    ...
    def test_large_radius_consistency(
        ...
        np.testing.assert_allclose(
            ...
            atol=1.0,
            rtol=0.01,
            err_msg="Metal and CPU should match for large radius",
        )
