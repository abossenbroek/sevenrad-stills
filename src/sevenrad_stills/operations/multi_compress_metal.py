"""
Metal-accelerated multi-generation compression for compound quality degradation.
...
- Metal GPU: Reserved for future parallelizable preprocessing operations

This implementation exists for API consistency within Metal-based image processing
pipelines on Apple Silicon. The iterative nature of multi-compression (iteration N
depends on N-1) combined with serial Huffman encoding makes GPU acceleration
impractical, as GPU↔CPU memory transfer overhead would exceed any potential gains.
...
if sys.platform != "darwin":
    msg = "multi_compress_metal operation is only available on macOS"
...
class MultiCompressMetalOperation(BaseImageOperation):
    """
    Apply JPEG compression multiple times to compound artifacts (Metal version).

    **Performance Note**: This operation is CPU-bound due to serial Huffman encoding.
    The Metal version exists for API consistency but delegates all work to PIL's
    highly optimized CPU implementation (libjpeg-turbo).

    **Implementation Philosophy:**
    Following NVIDIA nvJPEG's approach, JPEG encoding is optimally performed on CPU:
    - **Serial Operations (CPU)**: Huffman encoding, bitstream generation
    - **Metal GPU**: Reserved for future parallel operations (color conversion, DCT)

    This avoids the complexity of implementing full JPEG encoding on GPU while
    maintaining architectural coherence across Metal-accelerated operations.
    ...
    def apply(self, image: Image.Image, params: dict[str, Any]) -> Image.Image:
        ...
        **Implementation Note**: Despite being a Metal operation class, this
        ...
        inherently serial. Metal GPU acceleration would introduce memory transfer
