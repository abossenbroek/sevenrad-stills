...
class CompressionGPUOperation(BaseImageOperation):
    ...
    def benchmark_kernel_only(self, image: Image.Image, gamma: float) -> dict[str, Any]:
        """
        Prepare GPU state for kernel-only benchmarking.

        This method pre-allocates Taichi fields and loads data to GPU,
        ...
        Returns:
            ...
            - input_field: Taichi field with data loaded on GPU
            - output_field: Taichi field for results (on GPU)
            - gamma: Gamma value
        ...
        # Pre-load data to GPU (NOT timed in kernel benchmark)
        input_field.from_numpy(img_array)
    ...
    def run_kernel_only(self, prepared_data: dict[str, Any]) -> None:
        """
        Run ONLY the GPU kernel dispatch (no data transfers).
