"""
Metal-accelerated band swap operation using Taichi Metal backend.
...
Uses Taichi with Metal backend for maximum GPU performance on Apple Silicon.
"""
...
if sys.platform != "darwin":
    msg = "band_swap_metal operation is only available on macOS"
    raise ImportError(msg)

# Initialize Taichi with explicit Metal backend for Apple Silicon
ti.init(arch=ti.metal, default_fp=ti.f32)

# Constants (same as CPU and GPU versions)
MIN_TILE_COUNT = 1
MAX_TILE_COUNT = 50
MIN_TILE_SIZE = 0.01
MAX_TILE_SIZE = 1.0
...
def swap_channels(  # type: ignore[no-untyped-def]
    ...
    """
    Metal device function to swap channels at a single pixel.
...
def apply_band_swap_batch(  # type: ignore[no-untyped-def]
    ...
    """
    Metal kernel to process all tiles in a single batch.
...
class BandSwapMetalOperation(BaseImageOperation):
    """
    Metal-accelerated band/channel swapping operation using Taichi.
    ...
    accelerated using Taichi Metal kernels for maximum performance on Apple Silicon.
    ...
    Performance: Metal backend provides maximum GPU performance on Apple Silicon,
    with each tile's permutation parallelized across thousands of GPU threads.
    """
    ...
    def apply(self, image: Image.Image, params: dict[str, Any]) -> Image.Image:
        ...
        while the actual band permutation within each tile is executed on Metal GPU
        ...
        # Process all tiles in a single Metal kernel launch
